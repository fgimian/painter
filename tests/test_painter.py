...
def test_paint_support_applying_multiple_styles_at_once():
    assert (
...
def test_paint_support_nesting_styles():
    assert (
...
def test_paint_reset_all_styles_with_reset():
    assert (
...
def test_paint_alias_gray_to_grey():
    assert paint.grey('foo') == '\x1b[90mfoo\x1b[39m'


def test_paint_support_variable_number_of_arguments():
    assert paint.red('foo', 'bar') == '\x1b[31mfoo bar\x1b[39m'


def test_paint_support_falsy_values():
    assert paint.red(0) == '\x1b[31m0\x1b[39m'


@raises(AttributeError)
def test_paint_raises_exception_on_invalid_color():
...
def test_paint_enabled_does_not_output_colors_when_manually_disabled():
    paint.enabled = False
